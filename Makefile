# This Makefile is for the Alfred extension to perl.
#
...
#     ABSTRACT => q[Perl client for Ollama API]
#     AUTHOR => [q[Your Name <vkwasinski@gmail.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Alfred]
#     PREREQ_PM => { HTTP::Request=>q[0], JSON=>q[0], LWP::UserAgent=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Alfred.pm]
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Alfred.pm \
	lib/Alfred/HttpClient.pm \
	lib/Alfred/OllamaClient.pm
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  lib/Alfred.pm $(INST_MAN3DIR)/Alfred.$(MAN3EXT) \
	  lib/Alfred/HttpClient.pm $(INST_MAN3DIR)/Alfred::HttpClient.$(MAN3EXT) \
	  lib/Alfred/OllamaClient.pm $(INST_MAN3DIR)/Alfred::OllamaClient.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Perl client for Ollama API'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Your Name <vkwasinski@gmail.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Alfred' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Perl client for Ollama API",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Your Name <vkwasinski@gmail.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Alfred",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Alfred" VERSION="0.01">' > Alfred.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl client for Ollama API</ABSTRACT>' >> Alfred.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Your Name &lt;vkwasinski@gmail.com&gt;</AUTHOR>' >> Alfred.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Alfred.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="HTTP::Request" />' >> Alfred.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="JSON::" />' >> Alfred.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="LWP::UserAgent" />' >> Alfred.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-thread-multi-5.40" />' >> Alfred.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Alfred.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Alfred.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Alfred.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'alfred.pl' '$(INST_LIB)/alfred.pl' \
	  'lib/Alfred.pm' 'blib/lib/Alfred.pm' \
	  'lib/Alfred/HttpClient.pm' 'blib/lib/Alfred/HttpClient.pm' \
	  'lib/Alfred/OllamaClient.pm' 'blib/lib/Alfred/OllamaClient.pm' 
